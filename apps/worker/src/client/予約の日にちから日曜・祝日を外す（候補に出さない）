...
interface FormField {
  ...
  maxQuantity?: number;
  /**
   * type: 'date' のとき true にすると、営業日だけを並べた選択肢になる。
   * 日曜・祝日は最初から出ないので、定休日を選んでしまうことが起こらない。
   * 生年月日のような過去の日付には付けないこと（未来の営業日しか出なくなる）。
   */
...
// ─── 定休日（日曜・祝日） ─────────────────────────────────────────────
// 日祝は店が閉まっているので、フォームに出てはいけない。
// 「選べてしまってから怒られる」のは、お客様にとっていちばん嫌な形なので、
// 候補そのものを作らない。
//
// 祝日は法律で毎年決まるため計算では出せない（振替休日・国民の休日がある）。
// 内閣府の「国民の祝日について」の一覧を写したもの。2027年までしか入っていない。
// 一覧の範囲より先の日付は、判定せずそのまま出す（黙って弾くより、届いてから
// こちらでお返事したほうが安全）。表は毎年2月ごろに足すこと。
...
/** 日曜または祝日なら true。一覧の範囲外は false（＝営業日として扱う）。 */
function isClosedDay(d: Date): boolean {
  if (d.getFullYear() > HOLIDAY_TABLE_LAST_YEAR) return false;
  if (d.getDay() === 0) return true;
  return HOLIDAY_SET.has(ymd(d));
}

const WEEKDAY_JA = ['日', '月', '火', '水', '木', '金', '土'];

/**
 * 翌日から数えて、営業日だけを最大 days 日ぶん並べた選択肢を作る。
 * 当日を入れないのは、フォームが届いてから返事をするまでに時間がかかるため。
 */
function businessDayOptions(days = 60): string {
  const out: string[] = ['<option value="">選んでください</option>'];
  const cursor = new Date();
  cursor.setHours(0, 0, 0, 0);
  for (let i = 0; i < days; i++) {
    cursor.setDate(cursor.getDate() + 1);
    if (isClosedDay(cursor)) continue;
    const v = ymd(cursor);
    const label = `${cursor.getMonth() + 1}月${cursor.getDate()}日（${WEEKDAY_JA[cursor.getDay()]}）`;
    out.push(`<option value="${v}">${label}</option>`);
  }
  return out.join('');
}
...
function renderField(field: FormField): string {
  ...
  switch (field.type) {
    ...
    default:
      // 予約の日にちは、日曜・祝日を含まない営業日の一覧から選ばせる。
      // カレンダー入力では定休日を灰色にできないため、候補を作らない形にしている。
      if (field.type === 'date' && field.businessDays) {
        inputHtml = `<select
        name="${escapeHtml(field.name)}"
        id="field-${escapeHtml(field.name)}"
        class="form-select"${required}>${businessDayOptions()}</select>
        <p class="form-hint">日曜・祝日は定休日のため、候補に出ていません。</p>`;
        break;
      }
      inputHtml = `<input
...
function injectStyles(): void {
  ...
  style.textContent = `
    ...
    .form-hint { font-size: 12.5px; color: #6B7A72; margin: 6px 0 0; }
...
function validateForm(): string | null {
  ...
  // 念のための最後の砦。フォームを開いたまま日付をまたいだり、値を書き換えられた
  // ときでも、定休日のまま受け付けてしまわないようにする。
  for (const field of formDef.fields) {
    if (field.type !== 'date' || !field.businessDays) continue;
    ...
    if (isClosedDay(d)) {
      return `${field.label} が定休日（日曜・祝日）になっています。別の日をお選びください。`;
    }
  }
