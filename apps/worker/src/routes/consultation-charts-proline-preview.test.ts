...
function previewDb(options: { existingProline?: boolean; sameTimeSuzuki?: boolean } = {}) {
          ...
          return {
            first: async () => {
              if (sql.includes('FROM external_bookings')) {
                return options.existingProline ? { id: 'external-1', status: 'scheduled' } : null;
              }
              if (sql.includes('FROM bookings')) {
                return options.sameTimeSuzuki ? { id: 'booking-1', status: 'confirmed' } : null;
              }
              return { id: 'friend-1', display_name: 'SYK', picture_url: null };
            },
            run: async () => {
              state.writes += 1;
              return { success: true };
  ...
  test('通知を解析するだけで予約データを書き込まない', async () => {
    ...
    expect(await response.json()).toEqual({
      preview: {
        ...
        same_time_suzuki_booking: false,
      },
    });
    expect(state.prepared).toHaveLength(3);
    expect(state.writes).toBe(0);
  });

  test('取り込み済みと鈴木薬舗OSの同時刻予約を保存前に知らせる', async () => {
    const { db, state } = previewDb({ existingProline: true, sameTimeSuzuki: true });
    const response = await app().request(
      '/api/consultation-charts/friend-1/external-bookings/preview?account_id=account-1',
      {
        method: 'POST',
    ...
    expect(await response.json()).toMatchObject({
      preview: { already_imported: true, same_time_suzuki_booking: true },
    });
    expect(state.writes).toBe(0);
  });
